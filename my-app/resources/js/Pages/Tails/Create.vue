...
// No existing photos on a new tail, so PhotoUploadField's "remove existing"
// branch is unreachable here — only the file itself needs to be tracked.
...
const submit = () => {
    form.post(route('tails.store'), { forceFormData: true });
};
</script>
...
<template>
    ...
    <AdminLayout title="New Tail" :breadcrumb="{ label: 'Database', route: 'database.index' }">
